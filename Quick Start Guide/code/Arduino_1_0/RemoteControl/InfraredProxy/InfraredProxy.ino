...
class InfraredProxy {
  ...
  void read_line(EthernetClient& client, char* buffer, const int buffer_length) { // <label id="code.remote_control.read_line"/>
    int buffer_pos = 0;
  ...
  void receive_from_server(EthernetServer server) {
    ...
    char line[MAX_LINE];
    EthernetClient client = server.available(); // <label id="code.remote_control.server_available"/>
    if (client) {
