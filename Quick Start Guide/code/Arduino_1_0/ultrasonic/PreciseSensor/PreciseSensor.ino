...
unsigned long last_measurement = millis(); // <label id="code.input.measure_once_a_second"/>
...
void loop() {
  unsigned long current_millis = millis(); // <label id="code.input.start_measure_check"/>
  if (abs(current_millis - last_measurement) >= 1000) {
    current_temperature = get_temperature();
    last_measurement = current_millis;
  } // <label id="code.input.end_measure_check"/>
...
const float microseconds_per_cm() {
  return 1 / ((331.5 + (0.6 * current_temperature)) / 10000); // <label id="code.input.speed_of_sound"/>
}
