...
const float SENSOR_OFFSET = MOUNTING_GAP * MICROSECONDS_PER_CM * 2; // <label id="code.input.calc_offset"/>
...
void output_distance(const unsigned long duration) {
  ...
  Serial.print(microseconds_to_cm(duration)); // <label id="code.input.print_float"/>
  Serial.println(" cm");
}
